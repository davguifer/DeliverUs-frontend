  ...
  const renderPopular = ({ item }) => {
    return (
      <View style={styles.cardBody}>
        ...
        />
        <TextRegular style={{ alignItems: 'center' }} numberOfLines={2}>{item.description}</TextRegular>
  ...
  const renderHeaderPopular = () => {
    return (
      <View style={styles.container}>
        <Text style={{ fontSize: 27, color: 'black', margin: 40, fontFamily: 'Montserrat_600SemiBold' }}>
          ¡ESTOS SON LOS TOP 3 PRODUCTOS!
         </Text>
        <FlatList
          horizontal={true}
